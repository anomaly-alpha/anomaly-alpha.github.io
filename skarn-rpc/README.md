...
Candidate arbitration and rendering live in `features/presence/candidateSelector.js` and `features/presence/activityRenderer.js`. News is restricted to the 15 approved HTTPS RSS/Atom feeds in `data/news-sources.json`. The top-level `mode` is either `ambient` or `news`; `newsDwellMs` controls headline rotation and is currently 30 seconds. `freshnessMode` is normally `fresh` (six hours), while the local backfill mode uses `retroactive` to retain whatever the approved feeds expose. The cache remains bounded at 150 items, with the retroactive rollout capped at 10 items per source, and every rendered item keeps its real publication age. Feed polling remains a separate 15-minute operation. News mode remains news-only when no item exists, showing a bounded status instead of silently falling back to ambient phrases. Tests use only `test-fixtures/` and never query live feeds.

When explicitly enabled for rollout, the poller keeps bounded state in the ignored owner-only `data/news-state.json`. Fresh news interrupts ambient presence for the configured 30-second dwell, then ambient content resumes when no fresher candidate remains. No buttons, party data, or URLs are emitted.
