...
- **Emotional intelligence**: Keyword + sentiment-based emotion detection (happy/sad/anxious/angry/stressed). State stored in `user_emotional_context`. Generates tone directives for the AI system prompt. **Analyzer-sourced path (2026-08-08)**: analyzed messages (`>= 50` chars or `?`) source emotion/intensity/subtext from the message analyzer, mapped at the boundary via `mapAnalyzerEmotion`; the tone analyzer remains the short-message fallback and feeds the attention gate.
